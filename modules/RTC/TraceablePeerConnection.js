    ...
    */
    ...
    if (this._usesUnifiedPlan) {
        ...
        this.peerconnection.onaddstream = event => {
            if (this._maybeAddPending('onaddstream', event.stream) && 'remoteStreams' in this.peerconnection) {
                // ICC: iosrtc triggers add-stream before ICE is ready.
                // We delay adding the stream.
                this._pendingStreams.push([ event.stream.id ]);
            } else {
        ...
        this.peerconnection.onremovestream = event => {
            if (this._maybeRemovePending('onremovestream', event.stream)) {
                return;
            }
